...
- **Next.js 15 마이그레이션 대응** — 동적 라우트의 `params`가 Promise로 바뀐 변경사항에 맞춰 API 라우트 수정
- **검색 상태 복원** — 목록에서 상세 페이지로 이동 후 뒤로가기 시, 검색어·필터·스크롤 위치를 `sessionStorage`로 복원해 사용자가 검색을 처음부터 다시 하지 않도록 처리
